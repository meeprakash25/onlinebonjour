...
class AdminCategoriesController extends Controller
{
	...
	public function deletecategory(Request $request){
        ...
        $pro_to_cate = DB::table('products_to_categories')
            ->where('categories_id', $request->id)->get();
        foreach($pro_to_cate as $data){
            $product = DB::table('products')
                ->where('products_id', $data->products_id)->first();
            if ($product != null && $product->products_image !== null && app('files')->exists($product->products_image)) {
                app('files')->delete($product->products_image);
            }

            $pro_images = DB::table('products_images')
                ->where('products_id', $data->products_id)->first();
            if ($pro_images != null && $pro_images != null && app('files')->exists($pro_images->image)) {
                app('files')->delete($pro_images->image);
            }
             DB::table('products_images')
                ->where('products_id', $data->products_id)->delete();
             DB::table('products')
                ->where('products_id', $data->products_id)->delete();
        }
		...
		DB::table('categories')
			...
			->leftjoin('inventory_detail','inventory_detail.products_id','=','products.products_id')
			->leftjoin('flash_sale','inventory_detail.products_id','=','products.products_id')
			->leftjoin('products_images','products_images.products_id','=','products.products_id')
			->where('categories.categories_id', $request->id)->delete();
		//sub cat
		DB::table('categories')
			->leftjoin('categories_description','categories_description.categories_id','=','categories.categories_id')
			->leftjoin('products_to_categories','products_to_categories.categories_id','=','categories.categories_id')
			...
			->leftjoin('flash_sale','inventory_detail.products_id','=','products.products_id')
			->leftjoin('products_images','products_images.products_id','=','products.products_id')
			->where('categories.parent_id', $request->id)->delete();
			
		
		
		//delete products/categories
		$subcategories = DB::table('categories')->where('parent_id', $request->id)->get();
		foreach($subcategories as $subcategory){
			
			DB::table('categories_description')->where('categories_id', $subcategory->id)->delete();
