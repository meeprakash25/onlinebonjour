...
class AdminSiteSettingController extends Controller
{
    ...
    public function updateSetting(Request $request)
    {
        ...
        foreach ($request->all() as $key => $value) {
            ...
            //favicon logo
            if ($key == 'favicon') {
                if ($request->hasFile('favicon') and in_array($request->favicon->extension(), $extensions)) {
                    $dir = "resources/assets/images/site_images/";
                    if (!file_exists($dir) and !is_dir($dir)) {
                        mkdir($dir);
                    }
    ...
    public function mailchimp(Request $request)
    {
        if (session('website_setting_update') == 0) {
            ...
            $title              = array('pageTitle' => Lang::get("labels.mailchimp"));
            $result             = array();
            $settings           = DB::table('settings')->orderby('id', 'ASC')->get();
            $result['settings'] = $settings;
            return view("admin.mailchimp", $title)->with('result', $result);
        }
    }
}
