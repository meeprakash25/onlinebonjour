...
class ProductsController extends DataController
{
    ...
    public function shop(Request $request)
    {
        ...
        } elseif (!empty($request->manufacturer) and $request->manufacturer != 'all') {
            $slug              = $request->manufacturer;
            $manufacturer      = DB::table('manufacturers')->where('manufacturers_slug', $slug)->first();

            $manufacturers_id  = $manufacturer->manufacturers_id;
            $manufacturers_name  = $manufacturer->manufacturers_name;
            $manufacturers_slug  = $manufacturer->manufacturers_slug;;
        ...
        $result['manufacturers_name'] = $manufacturers_name;
        $result['manufacturers_slug'] = $manufacturers_slug;
        ...
        $data  = array('page_number' => $page_number, 'type' => $type, 'limit' => $limit, 'categories_id' => $categories_id, 'manufacturers_id' => $manufacturers_id, 'search' => $search, 'filters' => $filters, 'limit' => $limit, 'min_price' => $min_price, 'max_price' => $max_price);
    ...
    public function brands(Request $request)
    {
        ...
        //products
        $myVar                   = new DataController();
        $manufacturers           = $myVar->manufacturers();
        ...
        $countryCurrency              = DB::select(
            'SELECT * FROM settings WHERE name="sgd"'
        );
        $result['weeklySoldProducts'] = array('success' => '1', 'product_data' => $detail, 'message' => "Returned all products.", 'total_record' => count($detail));

        return view("brands", compact('title', 'countryCurrency'))->with('result', $result);
...
//        dd($request->all());
        ...
        $myVar                   = new DataController();
        $manufacturers           = $myVar->manufacturers();
        $manufacturers  = $manufacturers->where('manufacturers_slug','=',$search)->first();
        $result['manufacturers'] = $manufacturers;
        $result['search'] = $search;
//        dd($manufacturers);
        return view("brand", compact('manufacturers','search'))->with('result', $result);
