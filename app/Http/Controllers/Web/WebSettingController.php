...
class WebSettingController extends DataController
{
	...
	public function changeLanguage(Request $request){
		if($request->ajax()){
		   ...
		   //set language
		   $languages = DB::table('languages')->where('code','=',$request->local)->get();
	...
	public function changeCurrency(Request $request){
	    $currency = $request->change_countryCurrency;
        $countryCurrency = DB::select(
            'SELECT * FROM settings WHERE name="sgd"'
        );
        $USDCurrency = DB::select(
            'SELECT * FROM settings WHERE name="usd"'
        );
	    $request->session()->put('currency',$currency);
	    if ($currency == 'sgd'){
	        $request->session()->put('countryCurrency',$countryCurrency[0]->value);
        }else if($currency == 'usd'){
            $request->session()->put('countryCurrency',$USDCurrency[0]->value);
        }
	    return back()->with('message','currency Changed to '.($currency == "sgd" ? 'Singapore Dollar' : ($currency == "usd" ? 'US Dollar' : 'Nepali Rupee')));
    }
