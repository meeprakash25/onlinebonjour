...
class OrdersController extends DataController
{
	...
	public function checkout(Request $request){
		...
		if(count($result['cart'])==0){			
			...
			$shippingRate = DB::Select('select * from product_shipping_rates_by_location');

			return view("checkout", compact('title','shippingRate'))->with('result', $result);
		}
	...
	public function checkout_shipping_address(Request $request){
		...
		session(['shipping_address' => $address]);
        $shippingCost = DB::select(DB::raw('select shipping_price from product_shipping_rates_by_location where districts = :value'),array('value'=>$request->city));
        if($request->session('shipping_cost')){
            $request->session()->forget('shipping_cost');
            $request->session()->put('shipping_cost',$shippingCost[0]->shipping_price);
        }else{
            $request->session()->put('shipping_cost',$shippingCost[0]->shipping_price);
        }
...
//		$delivery = DB::table('zones')->where('zone_id', '=', session('shipping_address')->zone_id)->get();
		if (session('shipping_address')->zone_id == 1){
		    $delivery = "Province No. 1";
        }elseif (session('shipping_address')->zone_id == 2){
            $delivery = "Province No. 2";
        }elseif (session('shipping_address')->zone_id == 3){
            $delivery = "Province No. 3";
        }elseif (session('shipping_address')->zone_id == 4){
            $delivery = "Province No. 4";
        }elseif (session('shipping_address')->zone_id == 5){
            $delivery = "Province No. 5";
        }elseif (session('shipping_address')->zone_id == 6){
            $delivery = "Province No. 6";
        }elseif (session('shipping_address')->zone_id == 7){
            $delivery = "Province No. 7";
        }
...
//		$billing = DB::table('zones')->where('zone_id', '=', session('billing_address')->billing_zone_id)->get();
        if (session('billing_address')->billing_zone_id == 1){
            $billing = "Province No. 1";
        }elseif (session('billing_address')->billing_zone_id == 2){
            $billing = "Province No. 2";
        }elseif (session('billing_address')->billing_zone_id == 3){
            $billing = "Province No. 3";
        }elseif (session('billing_address')->billing_zone_id == 4){
            $billing = "Province No. 4";
        }elseif (session('billing_address')->billing_zone_id == 5){
            $billing = "Province No. 5";
        }elseif (session('billing_address')->billing_zone_id == 6){
            $billing = "Province No. 6";
        }elseif (session('billing_address')->billing_zone_id == 7){
            $billing = "Province No. 7";
        }
		...
		$shipping_method            		=   session('payment_method');
		$orders_status            			=   '1';
...
//		}
		...
		foreach($order as $data){
            ...
            try {
                ...
                $client->request('POST', $URL, [
                    'form_params' => [
                        ...
                        'message'     => 'Your order has been placed. It will be delivered to your shipping address soon.',
            ...
            if ($settings[11]->value) {
                $admin_phone = $settings[11]->value;
                try {
                    $client = new Client(); // initialize guzzle client
                    $client->request('POST', $URL, [
                        'form_params' => [
                            ...
                            'username'    => $SMS_USERNAME,
                            'password'    => $SMS_PASSWORD,
                            'message'     => 'New order received from ' . $delivery_firstname . ' ' . $delivery_lastname . '. View order on ' . url('/') . '/admin/login',
                            'destination' => $admin_phone,
                        ]
                    ]);
                } catch (\Exception $e) {
                    session()->flash('failed', $e->getMessage());
	...
	public function getPaymentMethods(){
		...
		//dd($payments_setting[0]);
		
		if($payments_setting[0]->cod_name=='0'){
