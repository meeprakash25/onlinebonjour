...
//		}
	...
	public function updatesinglecart(Request $request){
		...
		(($request->quantity)? ($customers_basket_quantity = str_replace('"','', $request->quantity)) : ($customers_basket_quantity = 1));
		...
		if($stocksToValid > $customers_basket_quantity){
			$customers_basket_quantity = intval($request->quantity);
		}
		//update into cart
		...
		if(!emptyArray($request->option_id) && count($request->option_id)>0){
